  ...
  '.json': {
      ...
      if (initValidation.error && initValidation.error.isJoi === true) {
        initValidation.error.details.forEach(detail => {
          errors.push({
            ruleId: 'bad-schema',
            severity: 2,
            message: initValidation.error.details.message,
            source: initValidation.error.annotate,
            path: initValidation.error.details.path
          });
        });
    ...
    console.log('init joi end');
      ...
      const retrySchema = joi.object({
        ...
        MaxAttempts: joi.number().integer().min(1).max(99999998).optional(),
        ...
        if (validation.error && validation.error.isJoi === true) {
          validation.error.details.forEach(detail => {
            errors.push({
              ...
              source: validation.error.annotate,
              path: detail.path
            });
          });
